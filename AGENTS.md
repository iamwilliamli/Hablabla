...
The user has now authorized OpenAI API integration for the companion's `/devices`
dashboard. Its optional Ask OpenAI panel sends one instruction and one selected
app/window name to the Responses API; it returns one bounded custom-function
proposal, reviewed in the browser and approved separately on the Mac. Preserve
native approval, local-only screenshots, manual controls, session isolation,
one-time request IDs, and cancellation of stale proposals. Meeting AI remains
separate. Read `apps/companion/LOCAL_DASHBOARD.md#optional-openai-action-proposals`.

...
Read the implementation status and ownership table in [TECH_STACK.md](TECH_STACK.md) before editing. The meeting UI is implemented; live local-AI generation verification is deferred by the user. Do not restart that verification unless asked. The user is separately working on the Mac companion: `apps/companion/` is reserved for that work, with proposed device UI, relay, and protocol boundaries in sections 19–23. Do not implement or overwrite those components from an unrelated frontend task. Preserve the meeting workspace at `/`; coordinate shared configuration and protocol changes with the relevant owner.

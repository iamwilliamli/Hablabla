...
test("bundled RPC mode rejects unsupported actions and unexpected launch arguments", mac, () => {
  for (const input of [{ op: "shell", account: "not-a-uuid" }, { op: "capture_display" }, { op: "capture_window" }, { op: "list_windows" }, { op: "activate_window" }, { op: "type_text" }, { op: "pointer" }, { op: "request_screen_permission" }, { op: "request_accessibility_permission" }, { op: "open_resource", path: "/tmp/unregistered.txt" }]) {

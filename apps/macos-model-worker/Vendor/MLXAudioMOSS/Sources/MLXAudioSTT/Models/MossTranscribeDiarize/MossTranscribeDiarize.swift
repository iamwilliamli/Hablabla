...
/// Materialized audio embeddings produced by Whisper Encoder, time merging,
/// and the MOSS VQ adaptor. A sequence of canonical 30-second chunks can be
/// concatenated later without running the audio encoder again.
public struct MossTranscribeDiarizeEncodedAudioChunk {
    public let embeddings: MLXArray
    public let sampleCount: Int

    public init(embeddings: MLXArray, sampleCount: Int) {
...
public final class MossTranscribeDiarizeBackbone: Module {
    ...
    func injectAudioFeatures(
        ...
        return try injectAudioEmbeddings(
            inputIds: inputIds,
...
private extension MossTranscribeDiarizeModel {
    ...
    func prepareGenerationInputs(
        encodedAudioChunks: [MossTranscribeDiarizeEncodedAudioChunk],
        prompt: String?
    ) throws -> PreparedGenerationInputs {
        guard !encodedAudioChunks.isEmpty else {
            throw STTError.invalidInput("At least one encoded audio chunk is required.")
        }
        let audioTokenCount = encodedAudioChunks.reduce(0) {
            $0 + $1.audioTokenCount
        }
        guard audioTokenCount > 0 else {
            throw STTError.invalidInput("Encoded audio chunks must contain audio tokens.")
        }

        let inputIds = try buildPrompt(
            audioTokenCount: audioTokenCount,
            prompt: prompt
        )
        let embeds = model.languageModel.embedTokens(inputIds)
        let inputsEmbeds = try model.injectAudioEmbeddings(
            inputIds: inputIds,
            inputsEmbeds: embeds,
            audioFeatures: encodedAudioChunks.map(\.embeddings)
        )
        eval(inputsEmbeds)
        return PreparedGenerationInputs(
            promptIds: inputIds,
            inputEmbeddings: inputsEmbeds,
            promptTokenCount: inputIds.dim(1),
            duration: Double(encodedAudioChunks.reduce(0) { $0 + $1.sampleCount })
                / Double(sampleRate)
        )
    }
    ...
    func generateSingleChunk(
        ...
        return try generateSinglePreparedInput(
            prepared,
            maxTokens: maxTokens,
            temperature: temperature,
            repetitionPenalty: repetitionPenalty,
    ...
    func generateSinglePreparedInput(
        _ prepared: PreparedGenerationInputs,
        maxTokens: Int,
        temperature: Float,
        repetitionPenalty: Float,
        repetitionContextSize: Int,
    ...
    func generateTokenIds(
        ...
        // The decode loop pipelines each next-token evaluation with asyncEval.
        // A cancellation can therefore unwind this scope before item() waits
        // for the most recently scheduled token. Drain the exact task-local
        // stream before its tensors are destroyed; Metal validation otherwise
        // reports buffers being deallocated while a command buffer still uses
        // them, and a later transcription commonly exposes the stale work.
        let generationStream = StreamOrDevice.default.stream
        defer {
            generationStream.synchronize()
        }
...
extension MossTranscribeDiarizeModel {
    public func encodeAudioChunk(
        audio: MLXArray
    ) throws -> MossTranscribeDiarizeEncodedAudioChunk {
        let wav = try audioToMono(audio)
        let sampleCount = wav.dim(0)
        guard sampleCount > 0,
              sampleCount <= WhisperAudioConfig.chunkLengthSamples else {
            throw STTError.invalidInput(
                "Encoded audio chunks must contain between 1 and \(WhisperAudioConfig.chunkLengthSamples) samples."
            )
        }

        let (inputFeatures, audioLengths, chunkMapping, featureLengths, _) = try preprocessAudio(wav)
        guard featureLengths.count == 1 else {
            throw STTError.invalidInput("A canonical encoded audio chunk cannot span multiple Whisper windows.")
        }
        ...
        guard let embeddings = audioFeatures.first,
              audioFeatures.count == 1 else {
            throw STTError.invalidInput("Expected one set of encoded audio embeddings.")
        }
        eval(embeddings)
        return MossTranscribeDiarizeEncodedAudioChunk(
            embeddings: embeddings,
            sampleCount: sampleCount
        )
    }

    public func streamingTranscribeEncodedChunks(
        _ encodedAudioChunks: [MossTranscribeDiarizeEncodedAudioChunk],
        offsetSeconds: Double,
        ...
        let sampleCount = encodedAudioChunks.reduce(0) { $0 + $1.sampleCount }
        ...
        let requestedMaxTokens = maxTokens
            ?? min(max(1, config.maxTokensPerPass), estimatedWindowTokens)
        let start = Date()
        let prefillStart = Date()
        let prepared = try prepareGenerationInputs(
            encodedAudioChunks: encodedAudioChunks,
            prompt: prompt
        )
        let prefillTime = Date().timeIntervalSince(prefillStart)
        return try generateSinglePreparedInput(
    ...
    public func streamingTranscribeWindow(
        audio: MLXArray,
        offsetSeconds: Double,
        config: StreamingConfig,
        maxTokens: Int? = nil,
    ...
    public static func combineChunkOutputs(_ outputs: [STTOutput], totalTime: TimeInterval) -> STTOutput {
        let text = outputs
            .map(\.text)
    ...
    static func parseSegments(
        ...
        // A tagged MOSS response can legitimately contain no spoken text. In
        // that case every matched segment is filtered out above; returning the
        // raw control-token stream as fallback text would surface timestamps
        // and speaker IDs as a transcript.
        if !matches.isEmpty {
            return segments
        }
        return [["start": offsetSeconds, "end": offsetSeconds + max(fallbackEnd, 0.0), "text": text]]

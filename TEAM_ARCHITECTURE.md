...
| Native speech | `apps/macos-model-worker/` and authenticated `apps/speech-gateway/` are merged; the web app now connects through server-only `/api/speech/*` proxies for MOSS uploads/polling/cancellation and Parakeet tickets | William owns worker verification; frontend owners maintain the browser capture and transcript experience |
...
| Checks | All 138 repository tests and workspace typechecks, plus three app-bundle checks, pass after the dashboard and speech frontend integrations; the speech proxy/upload UI pass a contract-compatible end-to-end browser check; real browser pairing, permission reporting, cancel/decline and approved native-to-browser image rendering were verified on macOS 26 | Native speech inference, live browser LLM generation, and live provider writes still require their target hardware/accounts |
